...
static int amp_calib(void* adev) {
    ...
    spkl_empty = !spkl_empty;

    int spkr_empty = 0;
    for (int i = 0; i < 4; i++) {
        spkr_empty |= handle.spkr.cal_r[i];
    }
    spkr_empty = !spkr_empty;

    // Workaround a empty TA partition: both spkr and spkl's TA data is 0x0
    handle.spkl.cal_valid = !spkl_empty;
    handle.spkr.cal_valid = !spkr_empty;
    ALOGI("spkl valid: %d, spkr valid: %d\n", handle.spkl.cal_valid, handle.spkr.cal_valid);
    handle.is_stereo = spkl_empty == spkr_empty;
#else
